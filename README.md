...
| ℹ️ YZ Açıklanabilirliği | Modelin şeffaflık analizi, SHAP grafikleri |
| ℹ️ Hakkında | Takım bilgileri, teknolojiler |
...
Aşağıdaki komutu kullanarak projenin çalışması için gereken Python paketlerini tek seferde yükleyebilirsiniz:
```bash
pip install -r requirements.txt
```
> **macOS için (Gerekiyorsa):** Apple Silicon veya Intel Mac kullanıyorsanız ve LightGBM/CatBoost'un hızlı çalışmasını istiyorsanız `brew install libomp` komutunu çalıştırabilirsiniz.

### 2. Arayüzün (Dashboard) Başlatılması
Modelin tüm performansını, grafiklerini incelemek ve test verisi üzerinden klinik varyant analizi yapmak için web arayüzünü (Streamlit) çalıştırın:
```bash
streamlit run app.py
```
> *(Bu komut varsayılan web tarayıcınızda otomatik olarak `http://localhost:8501` adresini açacaktır.)*

### 3. Yarışma Günü Toplu Test (Terminal Üzerinden)
Yarışma sırasında size verilecek olan yeni (gizli) verisetini terminalden tek bir komutla tahmin ettirebilirsiniz:

```bash
# Genel Değerlendirme İçin (Master Threshold):
python tahmin_uret.py --input test_verisi.csv --output sonuclar.csv

# Belirli Bir Panele Özel Tahmin İçin (Örn: KANSER):
python tahmin_uret.py --input test_kanser.csv --output sonuc_kanser.csv --panel KANSER
```

### 4. (Opsiyonel) Modeli Baştan Eğitmek
Sıfırdan tüm özellikleri üretip (Feature Engineering), 4 ana modeli ve Meta-Learner'ı eğitmek isterseniz aşağıdaki dosyayı çalıştırabilirsiniz (Eğitim Optuna nedeniyle ortalama 45 dakika sürer):
```bash
...
Eğitilen final modelin (v4.0) **Master Panel (OOF)** sonuçları aşağıdaki gibidir:
...
> **Not:** Sistem özellikle hastalık yapıcı (Patojenik) varyantları kaçırmamaya optimize edildiği için Recall (%94.5) oldukça yüksektir. Tüm fold ve test kırılımları detaylı şekilde `sonuclar/test_raporu.txt` dosyasında bulunmaktadır.

---

## Klasör Yapısı

Projeye ait en güncel dosya ağacı jürinin doğrudan çalıştırabileceği formattadır:
...
│   ├── veri_imputer.joblib            # Eksik veri doldurucu
│   ├── veri_scaler.joblib             # Ölçeklendirici
│   ├── model_kolonlari.joblib         # Kolon sıralaması
│   └── freq_maps.joblib               # Kategorik frekans haritaları
│
├── app.py                             # Streamlit Web Dashboard
├── kgm_med39_predictor.py             # Model Eğitim / Optimizasyon
├── tahmin_uret.py                     # Final Günü Tahmin Scripti
├── requirements.txt                   # Python bağımlılıkları
├── egitim.log                         # 45 Dakikalık Detaylı Optuna Logu
└── README.md                          # Bu dosya
```

> **Veri Güvenliği Notu:** Yarışma eğitim setleri (`veriler/` klasörü) boyut sınırları ve yarışma gizlilik politikası gereği GitHub reposuna `.gitignore` aracılığıyla özellikle yüklenmemiştir.
